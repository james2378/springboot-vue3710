		...
		<el-form ref="form" :model="form" status-icon label-width="120px" v-if="is_view()">
			...
			<el-col v-if="user_group === '管理员' || $check_field('get','adopter_number') || $check_field('add','adopter_number') || $check_field('set','adopter_number')" :xs="24" :sm="12" :lg="8" class="el_form_item_warp">
				<el-form-item label="领养者编号" prop="adopter_number">
					<el-input id="adopter_number" v-model="form_sub['adopter_number']" placeholder="请输入领养者编号"
							  v-if="user_group === '管理员' || (form_sub['adopter_id'] && $check_field('set','adopter_number')) || (!form_sub['adopter_id'] && $check_field('add','adopter_number'))"></el-input>
					<div v-else-if="$check_field('get','adopter_number')">{{form_sub['adopter_number']}}</div>
				</el-form-item>
			</el-col>
			<el-col v-if="user_group === '管理员' || $check_field('get','adopter_name') || $check_field('add','adopter_name') || $check_field('set','adopter_name')" :xs="24" :sm="12" :lg="8" class="el_form_item_warp">
				<el-form-item label="领养者姓名" prop="adopter_name">
					<el-input id="adopter_name" v-model="form_sub['adopter_name']" placeholder="请输入领养者姓名"
							  v-if="user_group === '管理员' || (form_sub['adopter_id'] && $check_field('set','adopter_name')) || (!form_sub['adopter_id'] && $check_field('add','adopter_name'))"></el-input>
					<div v-else-if="$check_field('get','adopter_name')">{{form_sub['adopter_name']}}</div>
				</el-form-item>
			</el-col>
...
<script>
	...
	export default {
        ...
        form_sub = this.events("submit_before", Object.assign({}, form_sub)) || form_sub;
				if(form_sub["adopter_id"]){
					// 提交事件
					this.$post("~/api/" + this.group_table + "/set?adopter_id=" + form_sub["adopter_id"],form_sub,(res)=>{
			...
			is_view(){
				...
				if(!bl){
					bl = this.$check_action('/adopter/table','add');
					console.log(bl ? "你有表格添加权限视作有添加权限" : "你没有表格添加权限");
				}
				if(!bl){
					bl = this.$check_action('/adopter/table','set');
					console.log(bl ? "你有表格添加权限视作有修改权限" : "你没有表格修改权限");
				}
				if(!bl){
					bl = this.$check_action('/adopter/view','add');
					console.log(bl ? "你有视图添加权限视作有添加权限" : "你没有视图添加权限");
				}
				if(!bl){
					bl = this.$check_action('/adopter/view','set');
					console.log(bl ? "你有视图修改权限视作有修改权限" : "你没有视图修改权限");
				}
				if(!bl){
					bl = this.$check_action('/adopter/view','get');
