		...
		<el-form ref="form" :model="form" status-icon label-width="120px" v-if="is_view()">
			...
			<el-col v-if="user_group === '管理员' || $check_field('get','publisher_number') || $check_field('add','publisher_number') || $check_field('set','publisher_number')" :xs="24" :sm="12" :lg="8" class="el_form_item_warp">
				<el-form-item label="发布者编号" prop="publisher_number">
					<el-input id="publisher_number" v-model="form_sub['publisher_number']" placeholder="请输入发布者编号"
							  v-if="user_group === '管理员' || (form_sub['publisher_id'] && $check_field('set','publisher_number')) || (!form_sub['publisher_id'] && $check_field('add','publisher_number'))"></el-input>
					<div v-else-if="$check_field('get','publisher_number')">{{form_sub['publisher_number']}}</div>
				</el-form-item>
			</el-col>
			<el-col v-if="user_group === '管理员' || $check_field('get','publisher_name') || $check_field('add','publisher_name') || $check_field('set','publisher_name')" :xs="24" :sm="12" :lg="8" class="el_form_item_warp">
				<el-form-item label="发布者姓名" prop="publisher_name">
					<el-input id="publisher_name" v-model="form_sub['publisher_name']" placeholder="请输入发布者姓名"
							  v-if="user_group === '管理员' || (form_sub['publisher_id'] && $check_field('set','publisher_name')) || (!form_sub['publisher_id'] && $check_field('add','publisher_name'))"></el-input>
					<div v-else-if="$check_field('get','publisher_name')">{{form_sub['publisher_name']}}</div>
				</el-form-item>
			</el-col>
...
<script>
	...
	export default {
        ...
        form_sub = this.events("submit_before", Object.assign({}, form_sub)) || form_sub;
				if(form_sub["publisher_id"]){
					// 提交事件
					this.$post("~/api/" + this.group_table + "/set?publisher_id=" + form_sub["publisher_id"],form_sub,(res)=>{
			...
			is_view(){
				...
				if(!bl){
					bl = this.$check_action('/publisher/table','add');
					console.log(bl ? "你有表格添加权限视作有添加权限" : "你没有表格添加权限");
				}
				if(!bl){
					bl = this.$check_action('/publisher/table','set');
					console.log(bl ? "你有表格添加权限视作有修改权限" : "你没有表格修改权限");
				}
				if(!bl){
					bl = this.$check_action('/publisher/view','add');
					console.log(bl ? "你有视图添加权限视作有添加权限" : "你没有视图添加权限");
				}
				if(!bl){
					bl = this.$check_action('/publisher/view','set');
					console.log(bl ? "你有视图修改权限视作有修改权限" : "你没有视图修改权限");
				}
				if(!bl){
					bl = this.$check_action('/publisher/view','get');
