		        ...
		        <div class="row">
		            <div class="col-12">
		                ...
		                <div class="diy_recommend">
		                    ...
		                    <div class="more_box">
		                        <router-link to="/pet_information/list" class="more">
		                            <span>更多</span>
		                        </router-link>
		                    </div>
		                    <div class="switch" @click="show_list_pet_information = !show_list_pet_information">
		                        <div class="switch_box">
		                            <span v-if="show_list_pet_information"> 切换表格 </span>
		                            <span v-else> 切换列表 </span>
		                        </div>
		                    </div>
		                </div>
		                <!--宠物信息推荐列表组件 -->
		                <list_pet_information v-if="show_list_pet_information" :list="list_pet_information" />
		                <div class="overflow-auto" v-else>
		                    <table id="list_diy" role="table" aria-busy="false"
		                           :aria-colcount="fields_pet_information.length"
		                           class="table b-table table-striped table-hover">
		                        <thead>
		                        <tr>
		                            <th v-for="(o,i) in fields_pet_information" :key="i">
		                                {{o.label}}
		                            </th>
		                        </tr>
		                        </thead>
		                        <tbody>
		                        <tr v-for="(o, i) in list_table_pet_information" :key="i" @click="to_details('pet_information',o,'pet_information_id')">
		                            <td v-for="(oj,n) in fields_pet_information">
		                                <img v-if="oj.type && oj.type == '图片' " :src="$fullUrl(o[oj.key])" alt=""
		                                     v-default-img="'/img/default.png'">
		        ...
		        <div class="row">
		            <div class="col-12">
		                ...
		                <div class="diy_recommend">
		                    ...
		                    <div class="more_box">
		                        <router-link to="/pet_knowledge/list" class="more">
		                            <span>更多</span>
		                        </router-link>
		                    </div>
		                    <div class="switch" @click="show_list_pet_knowledge = !show_list_pet_knowledge">
		                        <div class="switch_box">
		                            <span v-if="show_list_pet_knowledge"> 切换表格 </span>
		                            <span v-else> 切换列表 </span>
		                        </div>
		                    </div>
		                </div>
		                <!--宠物知识推荐列表组件 -->
		                <list_pet_knowledge v-if="show_list_pet_knowledge" :list="list_pet_knowledge" />
		                <div class="overflow-auto" v-else>
		                    <table id="list_diy" role="table" aria-busy="false"
		                           :aria-colcount="fields_pet_knowledge.length"
		                           class="table b-table table-striped table-hover">
		                        <thead>
		                        <tr>
		                            <th v-for="(o,i) in fields_pet_knowledge" :key="i">
		                                {{o.label}}
		                            </th>
		                        </tr>
		                        </thead>
		                        <tbody>
		                        <tr v-for="(o, i) in list_table_pet_knowledge" :key="i" @click="to_details('pet_knowledge',o,'pet_knowledge_id')">
		                            <td v-for="(oj,n) in fields_pet_knowledge">
		                                <img v-if="oj.type && oj.type == '图片' " :src="$fullUrl(o[oj.key])" alt=""
		                                     v-default-img="'/img/default.png'">
										<span v-else-if="oj.type && oj.type == 'UID' ">
											<span style="display: none">
...
<script>
	...
	import list_pet_knowledge from "@/components/diy/list_pet_knowledge.vue";
	...
	export default {
		...
		data() {
			return {
				...
				show_list_pet_information: true,
				list_pet_information: [],
				vm_arr_pet_information: [
					"pet_information_id",
					"pet_number",
					"title",
					"pet_variety",
				...
				show_list_pet_knowledge: true,
				list_pet_knowledge: [],
				vm_arr_pet_knowledge: [
					"pet_knowledge_id",
					"title",
					"cover",
					"video",
		...
		methods: {
			...
			// 获取宠物信息列表
			get_pet_information() {
				let url = "~/api/pet_information/get_list?";
				this.$get(url, {
					...
					if (json.result) {
						this.list_pet_information = json.result.list;
					}
				})
			},
			// 获取宠物知识列表
			get_pet_knowledge() {
				let url = "~/api/pet_knowledge/get_list?";
				this.$get(url, {
					"page": 1,
					"size": 8
				}, (json) => {
					if (json.result) {
						this.list_pet_knowledge = json.result.list;
					}
				})
			},
		...
		mounted() {
			this.get_pet_information();
			this.get_pet_knowledge();
			this.get_menu();
			this.get_slides();
			this.get_article();
			this.get_link();
		...
		computed: {
		    list_table_pet_information() {
		        var list = this.list_pet_information;
		        var list_table = [];
		        for (let i = 0; i < list.length; i++) {
		            list_table[i] = {};
		            this.vm_arr_pet_information.map((o) => {
		                // 第二个中括号是对象的属性
		    ...
		    list_table_pet_knowledge() {
		        var list = this.list_pet_knowledge;
		        var list_table = [];
		        for (let i = 0; i < list.length; i++) {
		            list_table[i] = {};
		            this.vm_arr_pet_knowledge.map((o) => {
		                // 第二个中括号是对象的属性
		                list_table[i][o] = list[i][o] || "";
		            });
		        }
