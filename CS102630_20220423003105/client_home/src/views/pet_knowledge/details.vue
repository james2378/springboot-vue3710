<template>
	<div class="diy_details page_pet_knowledge" id="pet_knowledge_details">
		<div class='warp'>
			<div class='container'>
				<div class='row'>
					<div class='col'>
						<div class='card_pet_knowledge'>
							<div_pet_knowledge :obj="obj"></div_pet_knowledge>
						</div>
					</div>
				</div>
			</div>
...
<script>
	import div_pet_knowledge from "@/components/diy/div_pet_knowledge.vue";
	...
	export default {
		...
		methods:{
			...
			add_hits(obj) {
				obj["hits"] = obj["hits"] + 1;
				var hits = obj["hits"];
				this.$post('~/api/pet_knowledge/set?pet_knowledge_id=' + obj["pet_knowledge_id"], {
					hits
				}, (res) => {
					if(res.result){
						...
						var body = {
							source_table: "pet_knowledge",
							source_field: "pet_knowledge_id",
							source_id: this.obj.pet_knowledge_id,
			...
			get_obj_after(json) {
				...
				if (json && json.result && json.result.obj) {
					...
					this.form_comment.source_id = this.obj.pet_knowledge_id;
				}
			},
			/**
			 * 初始化收藏状态
			...
			get_collect(obj) {
				...
				var query = {
					source_table: "pet_knowledge",
					source_field: "pet_knowledge_id",
					source_id: obj["pet_knowledge_id"],
					user_id
				};
				...
				if (this.state_praise) {
					...
					this.$get('~/api/praise/del?', query, (res) => {
						if(res.result){
							...
							this.$post('~/api/pet_knowledge/set?pet_knowledge_id=' + _this.obj["pet_knowledge_id"], {
								praise_len
							}, (res) => {
								if(res.result){
