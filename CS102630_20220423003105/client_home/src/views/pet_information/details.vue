<template>
	<div class="diy_details page_pet_information" id="pet_information_details">
		<div class='warp'>
			<div class='container'>
				<div class='row'>
					<div class='col'>
						<div class='card_pet_information'>
							<div_pet_information :obj="obj"></div_pet_information>
						</div>
					</div>
				</div>
			</div>
...
<script>
	import div_pet_information from "@/components/diy/div_pet_information.vue";
	...
	export default {
		...
		methods:{
			...
			add_hits(obj) {
				obj["hits"] = obj["hits"] + 1;
				var hits = obj["hits"];
				this.$post('~/api/pet_information/set?pet_information_id=' + obj["pet_information_id"], {
					hits
				}, (res) => {
					if(res.result){
			...
			get_obj_after(json) {
				...
				if (json && json.result && json.result.obj) {
					...
					this.form_comment.source_id = this.obj.pet_information_id;
				}
			},
			/**
			 * 初始化收藏状态
			...
			get_collect(obj) {
				...
				var query = {
					source_table: "pet_information",
					source_field: "pet_information_id",
					source_id: obj["pet_information_id"],
					user_id
				};
