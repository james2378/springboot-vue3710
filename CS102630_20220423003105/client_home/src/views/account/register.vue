						...
						<div class="card">
							...
							<div class="form_register">
								<b-form>
									...
									<!--领养者-->
									<form_adopter v-if="form.user_group === '领养者'" :form="form_sub"></form_adopter>
									<!--发布者-->
									<form_publisher v-if="form.user_group === '发布者'" :form="form_sub"></form_publisher>
...
<script>
	...
	/* 领养者 */
	import form_adopter from "@/components/diy/form_adopter";
	/* 发布者 */
	import form_publisher from "@/components/diy/form_publisher";
			...
			async submit_check(param){
				...
				if(!ret && user_group){
					if(user_group === "领养者"){
						...
						if(!ret && !form_sub["adopter_number"]){
							ret = "领养者编号不能为空!";
						}
						else if(!ret){
							res = await this.$get("~/api/adopter/count?", {"adopter_number": form_sub["adopter_number"]});
							if(res.result){
								ret = "领养者编号已存在!";
							}
						}
					...
					if(user_group === "发布者"){
						var form_sub = Object.assign({} ,this.form_sub);
						if(!ret && !form_sub["publisher_number"]){
							ret = "发布者编号不能为空!";
						}
						else if(!ret){
							res = await this.$get("~/api/publisher/count?", {"publisher_number": form_sub["publisher_number"]});
							if(res.result){
								ret = "发布者编号已存在!";
							}
						}
