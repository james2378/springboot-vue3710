<template>
	<div class="diy_details_box diy_div_adopter">
		<div class="warp">
			<div class="container">
		...
		methods: {
			...
			change_praise(obj) {
				...
				var query = {
					...
					source_field: "adopter_id",
					source_id: this.obj["adopter_id"],
					user_id
				};
				...
				if (this.state_praise) {
					...
					this.$get('~/api/praise/del?', query, (res) => {
						if(res.result){
							...
							this.$post('~/api/adopter/set?adopter_id=' + _this.obj["adopter_id"], {
								praise_len
							}, (res) => {
								if(res.result){
