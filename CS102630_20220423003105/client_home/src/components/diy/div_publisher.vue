<template>
	<div class="diy_details_box diy_div_publisher">
		<div class="warp">
			<div class="container">
		...
		methods: {
			...
			change_praise(obj) {
				...
				var query = {
					...
					source_field: "publisher_id",
					source_id: this.obj["publisher_id"],
					user_id
				};
				...
				if (this.state_praise) {
					...
					this.$get('~/api/praise/del?', query, (res) => {
						if(res.result){
							...
							this.$post('~/api/publisher/set?publisher_id=' + _this.obj["publisher_id"], {
								praise_len
							}, (res) => {
								if(res.result){
