<template>
	<div class="diy_details_box diy_div_pet_information">
		<div class="warp">
			<div class="container">
					...
					<div class="view praise_wrap">
						...
						{{ praiseLen }}
							...
							<div class="details_btn_wrap">
								<button class="btn btn-primary details_btn" v-if="$check_action('/pet_adoption/edit','add')" @click="to_form('/pet_adoption/edit')" >领养 </button>
								<button class="btn btn-primary details_btn" v-if="$check_action('/pet_information/details','set')" @click="change_praise()" >点赞</button>
								<button class="btn btn-primary details_btn" v-if="$check_action('/pet_information/details','set')" @click="change_collect()" >收藏</button>
							</div>
						<!-- 富文本 -->
		...
		methods: {
			...
			change_praise(obj) {
				...
				var query = {
					...
					source_field: "pet_information_id",
					source_id: this.obj["pet_information_id"],
					user_id
				};
				...
				if (this.state_praise) {
					...
					this.$get('~/api/praise/del?', query, (res) => {
						if(res.result){
							...
							this.$post('~/api/pet_information/set?pet_information_id=' + _this.obj["pet_information_id"], {
								praise_len
							}, (res) => {
								if(res.result){
			...
			change_collect() {
				...
				if (this.state_collect) {
					...
					query.title = this.obj.title
					query.img = this.obj.photo
